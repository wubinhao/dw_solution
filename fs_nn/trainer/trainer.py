...
from .utils import MODE, use_diff, format_log_hooks, filter_by_specified_keywords
...
class Trainer(object):
    ...
    @use_diff
    def train_early_stop(self, target_data_split_name="train", hooks_set=None,
                         valid_mode='val', early_stop_rounds=50, base_score=0):
        hooks_set = hooks_set or self.hooks_in_train

        self.ctx.check_data_split(target_data_split_name)

        self._run_routine_early_stop(MODE.TRAIN, hooks_set, target_data_split_name,
                                     valid_mode=valid_mode, early_stop_rounds=early_stop_rounds,
    ...
    def _run_routine_early_stop(self, mode, hooks_set, dataset_name=None,
                                valid_mode='val', early_stop_rounds=50, base_score=None
                                ):
        """Run the hooks_set and maintain the mode
        ...
        for epoch_i in range(self.ctx.get(
            ...
            # self.ctx.append_mode(valid_mode)
            # self.ctx.track_used_dataset(valid_mode)
            # val_hooks_set = self.hooks_in_eval

            # self._run_routine(MODE.TEST, val_hooks_set, dataset_name=valid_mode)
            # val_eval_metrics = self.ctx.eval_metrics
            # tr_eval_metrics = self.evaluate(target_data_split_name=mode)
            tr_eval_metrics = None
            val_eval_metrics = self.evaluate(target_data_split_name=valid_mode)


            if 'val_mse' in val_eval_metrics:
                val_score = val_eval_metrics['val_mse']
            elif 'val_acc' in val_eval_metrics:
                val_score = 1 - val_eval_metrics['val_acc']
            else:
                val_score = 1 / 0

            normal_score = round((base_score - val_score) / base_score * 100, 2) \
                if base_score is not None else 'bs_is_None'

            if val_score < best_val_score:
                self.ctx.best_model = copy.deepcopy(self.ctx.model)
                best_val_score = val_score
                best_normal_score = normal_score
                not_imp_rounds = 0

            not_imp_rounds += 1
            if not_imp_rounds > early_stop_rounds:
                self.ctx.model = self.ctx.best_model
                print('early stop at ', epoch_i, best_normal_score, val_eval_metrics)

                break

            print('opt log ', epoch_i, not_imp_rounds, normal_score, val_eval_metrics, tr_eval_metrics)
